...
namespace Assets.HeroEditor4D.FantasyInventory.Scripts
{
	public class ItemCollection : MonoBehaviour
	{
        ...
        public void OnValidate() => SmartSetup();

        public void Initialize() => Dict = UserItems.Union(GeneratedItems).ToDictionary(i => i.Id, i => i);
        ...
        private void SmartSetup()
        {
            foreach (var item in UserItems)
            {
                ...
                if (item.SpriteId.IsEmpty())
                {
                    ...
                    else Debug.LogWarning($"Suggestions for {item.Id}: " + string.Join(", ", variants.Select(i => $"{i.Path}")));
                }
                else if (IconCollection.Icons.All(i => i.Id != item.SpriteId)) Debug.LogWarning($"Icon not found for {item.SpriteId}!");
            }
        }
	}
}