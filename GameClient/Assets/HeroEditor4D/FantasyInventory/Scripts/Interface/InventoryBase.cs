...
namespace Assets.HeroEditor4D.FantasyInventory.Scripts.Interface
{
    ...
    public class InventoryBase : ItemWorkspace
    {
        ...
        public void Initialize(ref List<Item> playerItems, ref  List<Item> equippedItems, int bagSize, Action onRefresh)
        {
            ...
            if (!Equipment.SelectAny() && !Bag.SelectAny())  ItemInfo.Reset();
        }

        private void OnDoubleClick(Item item)
        {
            SelectItem(item);
            if (Equipment.Items.Contains(item)) Remove();
            else if (CanEquipSelectedItem()) Equip();
        }
        public void PanelFadeIn()
        {
            CanvasGroup.alpha = 0f;
            RectTransform.transform.localPosition = new Vector3(500f, -1000, 0f);
            RectTransform.DOAnchorPos(new Vector2(500f, 0f), fadeTime, false).SetEase(Ease.InOutQuint);
        ...
        public void Craft()
        {
            ...
            if (CanCraft(materials))
            {
                ...
                if (existed == null)  Bag.Items.Add(new Item(itemId));
                else existed.Count++;
            ...
            else Debug.Log("No materials.");
        }

        public void Use() => Use(UseSound);
        ...
        public Item Assemble()
        {
            if (SelectedItem != null && SelectedItem.Params.Type == ItemType.Fragment && SelectedItem.Count >= SelectedItem.Params.FindProperty(PropertyId.Fragments).ValueInt)
            {
                ...
                if (existed == null) Bag.Items.Add(crafted);
                else existed.Count++;
        ...
        public override void Refresh()
        {
            ...
            if (receipt)
            {
                ...
                else LearnButton.SetActive(true);
            }
        ...
        private bool CanEquipSelectedItem() => Bag.Items.Contains(SelectedItem) && Equipment.Slots.Any(i => i.Supports(SelectedItem));
        ...
        private bool CanCraft(List<Item> materials) => materials.All(i => Bag.Items.Any(j => j.Hash == i.Hash && j.Count >= i.Count));
        ...
        private void AutoRemove(List<Item> items, int max = 1)
        {
            ...
            if (sum == max) MoveItemSilent(items.LastOrDefault(i => i.Id != SelectedItem.Id) ?? items.Last(), Equipment, Bag);
        }
    }
}